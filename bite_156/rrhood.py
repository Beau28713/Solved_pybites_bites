            ...
            if isinstance(character, tuple):
                for syn in character:
                    if syn.lower() in line.lower():
                        character_index[character[0].lower()] = character_index.get(character[0].lower(), []) + [idx]
                        break
            else:
                if character.lower() in line.lower():
                    character_index[character.lower()] = character_index.get(character.lower(), []) + [idx]
    return {k: sorted(v) for k, v in character_index.items()}
