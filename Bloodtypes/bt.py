...
def check_bt(donor, recipient):
    ...
    """
    # Convert donor to Bloodtype enum
    if isinstance(donor, Bloodtype):
        donor_bt = donor
    elif isinstance(donor, int):
        if donor in range(8):
            donor_bt = Bloodtype(donor)
        else:
            raise ValueError("Donor blood type integer must be in range 0-7.")
    elif isinstance(donor, str):
        if donor in blood_type_text:
            donor_bt = blood_type_text[donor]
        else:
            raise ValueError("Donor blood type string is not valid.")
    else:
        raise TypeError("Donor blood type must be of type int, str, or Bloodtype.")

    # Convert recipient to Bloodtype enum
    if isinstance(recipient, Bloodtype):
        recipient_bt = recipient
    elif isinstance(recipient, int):
        if recipient in range(8):
            recipient_bt = Bloodtype(recipient)
        else:
            raise ValueError("Recipient blood type integer must be in range 0-7.")
    elif isinstance(recipient, str):
        if recipient in blood_type_text:
            recipient_bt = blood_type_text[recipient]
        else:
            raise ValueError("Recipient blood type string is not valid.")
    else:
        raise TypeError("Recipient blood type must be of type int, str, or Bloodtype.")

    # Check compatibility
    compatibility = _particular_antigen_comp(donor_bt.value, recipient_bt.value)
    return all(x >= 0 for x in compatibility)
